				...
				<table aria-describedby="sample-table-2_info" id="sample-table-2" class="table table-striped table-bordered table-hover dataTable">
					...
					<thead>
						<tr role="row">
							...
							<th aria-label="beneficio: activate to sort column ascending" style="width: 200px;" colspan="1" rowspan="1" aria-controls="sample-table-2" tabindex="1" role="columnheader" class="sorting">BLOQUES</th>
...
jQuery(function($) {
	...
	function fillTable(){
		...
		$.post(obj.getValue(0) + "data/", {o:1000, t:2000, c:nc, p:10, from:0, cantidad:0,s:''},
			function(json){
					...
					$.each(json, function(i, item) {
						...
						tB +='';
						tB +='				<td class=" ">';
						tB +='					<a href="#" >'+padl(item.idbeneficio,4)+'</a>';
						tB +='				</td>';
						tB +='				<td class=" " >'+item.beneficio+'</td>';
						tB +='				<td class=" ">';
					...
					$(".delBeneficio").on("click",function(event){
						...
						if (resp){
							//alert(arr[1]);
							obj.setIsTimeLine(false);
				            $.post(obj.getValue(0) + "data/", {o:1000, t:2, c:arr[1], p:2, from:0, cantidad:0, s:''},
				            function(json) {
				            		if (json[0].msg=="OK"){
										onClickFillTable();
				        			}else{
	...
	function getPropBeneficio(IdConcepto){
		var nc = localStorage.nc; //.split("@");
		$.post(obj.getValue(0) + "cat-beneficios-prop/", {
				user: nc,
				idbeneficio: IdConcepto
			},
			function(html) {
