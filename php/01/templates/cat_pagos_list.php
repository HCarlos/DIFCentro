...
<div class="widget-box ">
	...
	<div class="widget-header widget-header-flat">
			...
			<span class="widget-toolbar">
				<button class="btn btn-minier btn-purple" id="btnAddRegistry">
					<i class="icon-plus-sign white"></i> Generar Orden de Pago
				</button>
			</span>
	...
	<div class="widget-body">
		<div class="widget-main padding-4">
			...
			<table aria-describedby="sample-table-2_info" id="sample-table-2" class="table table-striped table-bordered table-hover dataTable">
				...
				<thead>
					<tr role="row">
						<th aria-label="idcontribucion: activate to sort column ascending" style="width: 10px;" colspan="1" rowspan="1" aria-controls="sample-table-2" tabindex="0" role="columnheader" class="sorting" >ID</th>
						...
						<th aria-label="contribuyente: activate to sort column ascending" style="width: 120px;" colspan="1" rowspan="1" aria-controls="sample-table-2" tabindex="2" role="columnheader" class="sorting">CONTRIBUYENTE</th>
						<th aria-label="conceptp: activate to sort column ascending" style="width: 120px;" colspan="1" rowspan="1" aria-controls="sample-table-2" tabindex="3" role="columnheader" class="sorting">CONCEPTOS</th>
						<th aria-label="cantidad: activate to sort column ascending" style="width: 10px;" colspan="1" rowspan="1" aria-controls="sample-table-2" tabindex="4" role="columnheader" class="sorting">CANT</th>
						<th aria-label="importe: activate to sort column ascending" style="width: 50px;" colspan="1" rowspan="1" aria-controls="sample-table-2" tabindex="5" role="columnheader" class="sorting">IMPORTE</th>
						<th aria-label="status: activate to sort column ascending" style="width: 50px;" colspan="1" rowspan="1" aria-controls="sample-table-2" tabindex="6" role="columnheader" class="sorting">STATUS</th>
...
jQuery(function($) {
	...
	function gotoPropIngreso(IdContribucion){
        $("#contentProfile").html("");
	...
	function getPagos(){
		...
		$.post(obj.getValue(0) + "data/", {o:1005, t:2010, c:nc, p:11, from:0, cantidad:0,s:' order by idcontribucion desc '},
			function(json){
				if (json.length>0){
					tcant = 0;
					ttotal = 0;
		           ...
		           $.each(json, function(i, item) {
						...
						$("#sample-table-2 > tbody").append(st);
					...
					$(".delGenContrib").on("click",function(event){
						...
						if (resp){
							//alert(arr[1]);
							obj.setIsTimeLine(false);
				            $.post(obj.getValue(0) + "data/", {o:1005, t:2, c:arr[1], p:2, from:0, cantidad:0, s:'order by fecha desc'},
				            function(json) {
				            		if (json[0].msg=="OK"){
										getPagos();
				        			}else{
	...
	function jsNewConcepto(datosServer) {
		...
		if ( ( ms[1] == 'PAGOCONTRIB' ) && ( localStorage.nc == ms[4] ) ) {
			fillTable();
		}
