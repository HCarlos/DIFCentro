...
$objPHPExcel = $objReader->load("templates/_fmt_rep_secundaria_esp_arji_1.xlsx"); //cargamos el archivo excel (extensión *.xlsx)
...
foreach ($arrAlu as $i => $value) {
	...
	if ( count($result)>0  ){
		...
		$fl=4;
		for ($l=0;$l<count($arrIdGruMat); ++$l){
			$mat = $f->getQuerys(56,"idgrualu=".$arrAlu[$i]."&numval=10&idgrumat=".$arrIdGruMat[$l],0,0,0,array(),' and padre <= 0 and (idmatclas in (1,2,3,4,5) ) order by orden_historial asc ');
			if ( count($mat)>0 ){
				$cal =  $A->FormatCal($mat[0]->cal,$mat[0]->con,$mat[0]->ina);
				$oS->setCellValueByColumnAndRow($fl,$k, $cal);
				++$fl;
...
$fileout= "reporte-calif-secundaria-esp-arji-".$ti.".xlsx";
