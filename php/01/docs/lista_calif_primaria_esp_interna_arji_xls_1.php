...
$mats = $f->getCombo(1,"idgrupo=".$idgrupo."&idciclo=".$idciclo,0,0,24,' and idioma = 0  order by orden_historial asc '); 
...
foreach ($arrAlu as $i => $value) {
	...
	if ( count($result)>0 ){
		...
		$fl=4;
		for ($l=0;$l<count($arrIdGruMat); ++$l){
			$mat = $f->getQuerys(56,"idgrualu=".$arrAlu[$i]."&numval=9&idgrumat=".$arrIdGruMat[$l],0,0,0,array(),' and idioma = 0 order by orden_historial asc ');
			if ( count($mat)>0 ){
