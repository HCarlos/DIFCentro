...
$objPHPExcel = $objReader->load("templates/_fmt_rep_primaria_ing_arji_1.xlsx"); //cargamos el archivo excel (extensión *.xlsx)
...
$mats = $f->getCombo(1,"idgrupo=".$idgrupo."&idciclo=".$idciclo,0,0,24,' and idioma = 1  and orden_historial <= 100 order by orden_historial asc '); 
...
foreach ($arrAlu as $i => $value) {
	...
	if ( count($result)>0  ){
		...
		$fl=4;
		for ($l=0;$l<count($arrIdGruMat); ++$l){
			$mat = $f->getQuerys(56,"idgrualu=".$arrAlu[$i]."&numval=9&idgrumat=".$arrIdGruMat[$l],0,0,0,array(),' and idioma = 1 order by orden_historial asc ');
			if ( count($mat)>0 ){
		...
		$fl=4;
		for ($l=0;$l<count($arrIdGruMat); ++$l){
			$mat = $f->getQuerys(56,"idgrualu=".$arrAlu[$i]."&numval=10&idgrumat=".$arrIdGruMat[$l],0,0,0,array(),' and idioma = 1  and orden_historial <= 100 order by orden_historial asc ');
			if ( count($mat)>0 ){
				//$cal =  $A->FormatCal($mat[0]->cal,$mat[0]->con,$mat[0]->ina);
				//$oS->setCellValueByColumnAndRow($fl,$k, $cal);
				if ( in_array(intval($mat[0]->idmatclas) , $os) ){
